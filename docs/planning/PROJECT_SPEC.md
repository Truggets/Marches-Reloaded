...
| M3 | Level 1 character creation | Full happy-path single-class L1 build (species/class/background/abilities/skills/equipment/spells), saved to the player's account. | **Built, verified locally** (2026-07-18) — server: `characters` table (additive migration) + `/api/characters` CRUD behind `requireAuth` with owner/admin checks, smoke-tested with two real accounts. Client: 7-step wizard (Class → Origin → Ability Scores [roll or manual entry] → Skills → Equipment → Spells → Name) built on the M2 `@data` query layer, storing choices (not computed stats) in a schema shaped to survive M6 multiclassing (`classes: []` array from day one). Full happy path (Human Sage Wizard 1) walked live in a browser end-to-end: register → create → list → delete, all against the real API. One rules bug caught and fixed during this pass: the Spells step was offering the class's entire spell list (up to 9th level) instead of only 1st-level spells choosable at character level 1. Not yet deployed — pending prod migration + push/deploy go-ahead. |
